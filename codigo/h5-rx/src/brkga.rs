...
impl BRKGA {
    ...
    fn evolve_population(&mut self, generation: usize) -> Vec<Individual> {
        ...
        if !self.elite_history.is_empty() {
            let dm_cromossome = self.compute_dm_rx();
    ...
    /// DMC-GRASP-RX: multidimensional x-means; pick a random cluster's mean.
    fn compute_dm_rx(&mut self) -> Vec<f32> {
        let max_clusters = 40;
        let clusters = xmeans_fit(&self.elite_history, max_clusters, &mut self.rng);
        let cluster_idx = self.rng.gen_range(0..clusters.len());
