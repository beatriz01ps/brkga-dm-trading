...
impl BRKGA {
    ...
    fn evolve_population(&mut self, generation: usize) -> Vec<Individual> {
        ...
        if !self.elite_history.is_empty() {
            let dm_cromossome = self.compute_dm_std();
    ...
    fn compute_dm_std(&mut self) -> Vec<f32> {
        let threshold: f32 = 0.685;
        let n = self.elite_history.len() as f32;
        let mut result = Vec::with_capacity(self.cromossome_size);
        for i in 0..self.cromossome_size {
            let mean: f32 = self.elite_history.iter().map(|c| c[i]).sum::<f32>() / n;
            let variance: f32 = self.elite_history.iter().map(|c| (c[i] - mean).powi(2)).sum::<f32>() / n;
            let std_dev = variance.sqrt();
            let value = if std_dev > threshold { self.rng.gen_range(0.0..1.0) } else { std_dev };
            result.push(value);
        }
        result
    }
