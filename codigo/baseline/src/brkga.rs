...
fn init_csv(path: &str) {
    ...
    writeln!(f, "generation,train_best,train_median,train_worst,val_best,gen_time_s,total_time_s").ok();
}

fn append_csv(path: &str, gen: usize, train_best: f32, train_median: f32,
              train_worst: f32, val_best: f32, gen_time: Duration, total_time: Duration) {
    let mut f = fs::OpenOptions::new().append(true).open(path).unwrap();
    writeln!(f, "{},{:.2},{:.2},{:.2},{:.2},{:.2},{:.2}",
        gen, train_best, train_median, train_worst, val_best,
        gen_time.as_secs_f32(), total_time.as_secs_f32()).ok();
}
