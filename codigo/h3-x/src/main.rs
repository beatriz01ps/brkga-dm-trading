...
fn main() {
    ...
    let frac_bot: f32 = 0.10;  // Î³ = 10% mutantes (paper seÃ§Ã£o 4.4)
    let frac_top: f32 = 0.25;  // Î² = 25% elite (paper seÃ§Ã£o 4.4)
    let pop_size : usize = 30000; // N = 30000 (paper seÃ§Ã£o 4.4)
    let max_iter: usize = 1000;   // itermax = 1000 (paper seÃ§Ã£o 4.4)
    let elit_rate : f32 = 0.70;   // Î± = 70% (paper seÃ§Ã£o 4.4)
...
fn run_experiment(candles: Vec<Candlestick>, seed: u64, config: BrkgaConfig) {
    ...
    let backtest_engine = Backtest::new(candles, 12, 0.005, 0.0001); // taxa 0.01% (paper seÃ§Ã£o 4.5)
    let mut brkga = BRKGA::new(seed, 35, config, // 35 genes: 1 alavancagem + 17 long + 17 short (paper seÃ§Ã£o 3.6)
