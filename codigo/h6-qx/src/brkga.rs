...
impl BRKGA {
    ...
    fn evolve_population(&mut self, generation: usize) -> Vec<Individual> {
        ...
        if !self.elite_history.is_empty() {
            let dm_cromossome = self.compute_dm_qx();
    ...
    /// DMC-GRASP-QX: multidimensional x-means; cycle through clusters in round-robin order.
    fn compute_dm_qx(&mut self) -> Vec<f32> {
        let max_clusters = 40;
        let clusters = xmeans_fit(&self.elite_history, max_clusters, &mut self.rng);
        self.round_robin_idx = (self.round_robin_idx + 1) % clusters.len();
        let selected = &clusters[self.round_robin_idx];
        let n = selected.len() as f32;
        (0..self.cromossome_size)
            .map(|i| selected.iter().map(|&j| self.elite_history[j][i]).sum::<f32>() / n)
            .collect()
    }
